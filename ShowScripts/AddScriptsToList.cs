...
namespace ShowScripts
{
    class AddScriptsToList
    {
        ...
        public static void Do(ref List<string> dynamizationList, ref List<string> eveList, ref List<ScreenDynEvents> screenDynEventList, HmiScreenComposition screens, ref List<string> csvStringP, ref string screenName, string whereCondition = "", string sets = "", bool deepSearch = false)
        {
            string _screenName = "";
            if(screenName == ".*")
            {             
        ...
        static public List<List<string>> GetAllMyAttributesDynPropEves(IEngineeringObject obj, bool deepSearch, List<string> whereConditions, List<string> sets)
        {
            ...
            foreach (var itemsDyn in dynamizations)
            {
                if (itemsDyn.Value.Count == 2 && (obj is HmiScreen || obj is HmiScreenItemBase))
                {
                    ...
                    tempListDyn.Insert(1, "function " + objectName + "_" + itemsDyn.Key + "_Trigger() {" + itemsDyn.Value[1] + Environment.NewLine + "}");
                }
                ...
                if (itemsDyn.Value.Count == 1 && (obj is HmiScreen || obj is HmiScreenItemBase))
                {
                    ...
                    tempListDyn.Add("function " + objectName + "_" + itemsDyn.Key + "_Trigger() {" + itemsDyn.Value[0] + Environment.NewLine + "}");
                }
            ...
            foreach (var itemsPropEve in propertyEvents)
            {
                if (itemsPropEve.Value.Count == 2 && (obj is HmiScreen || obj is HmiScreenItemBase))
                {
                    ...
                    tempListPropEve.Insert(1, Environment.NewLine + "export function " + objectName + "_" + itemsPropEve.Key + "_OnPropertyChanged() {" + itemsPropEve.Value[1] + Environment.NewLine + "}");
                }
                ...
                if (itemsPropEve.Value.Count == 1 && (obj is HmiScreen || obj is HmiScreenItemBase))
                {
                    ...
                    tempListPropEve.Add("export function " + objectName + "_" + itemsPropEve.Key + "_OnPropertyChanged() {" + itemsPropEve.Value[0] + Environment.NewLine + " }");
                }
            ...
            if (deepSearch)
            {
                ...
                foreach (IEngineeringObject item in objProps)
                {
                    if (item.GetType().Name != "MultilingualText")
                    {
                        ...
                        {
                            tempListDyn.Add("function " + objectName + dyn);
                        }
                        int index = 0;
                        foreach (var propEve in nodeDynPropEve[1])
                        {
                            if (index != 0)
                            {
                                tempListPropEve.Add(Environment.NewLine + "export function " + obj.GetAttribute("Name") + propEve);
                            }
                            else
                            {
        ...
        static public List<string> GetAllMyAttributesEve(IEngineeringObject obj, List<string> whereConditions, List<string> sets)
        {
            ...
            foreach (var itemsEve in events)
            {
                if (itemsEve.Value.Count == 2)
                {
                    ...
                    tempListEve.Insert(1, Environment.NewLine + "export async function " + objectName + "_" + itemsEve.Key + "() {" + itemsEve.Value[1] + Environment.NewLine + "}");
                }

                if (itemsEve.Value.Count == 1)
                {
                    tempListEve.Add(Environment.NewLine + "//eslint-disable-next-line camelcase");
                    tempListEve.Add("export async function " + objectName + "_" + itemsEve.Key + "() {" + itemsEve.Value[0] + Environment.NewLine + "}");
                }
            }
            return tempListEve;
        }
